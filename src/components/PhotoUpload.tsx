    ...
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {photos.map((photo, index) => (
            <div key={index} className="relative aspect-square rounded-xl overflow-hidden group">
              ...
              />
              {index === 0 && (
                <div className="absolute top-2 left-2 bg-[#0066cc] text-white text-xs px-2 py-0.5 rounded-full">
                  Main
                </div>
              )}
              <button
      ...
      {photos.length < 5 && (
        <button
          ...
          disabled={uploading}
          className="w-full border-2 border-dashed border-gray-300 hover:border-[#0066cc] rounded-xl p-6 flex flex-col items-center gap-2 transition-colors disabled:opacity-50"
        >
          {uploading ? (
